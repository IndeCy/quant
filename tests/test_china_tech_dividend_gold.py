...
from examples import china_tech_dividend_gold_study as study
...
def test_definition_is_fixed_equal_weight_without_timing() -> None:
    ...
    assert definition["portfolio"]["ranking"] is False
...
def test_gate_rejects_when_return_does_not_clear_sp500() -> None:
    ...
    metrics = {
        ...
        study.TECH_DIVIDEND_ID: {
            key: {**base, "max_drawdown": -0.24, "sharpe": 0.80}
            for key in candidate
        },
        study.TECH_ID: {
            key: {**base, "max_drawdown": -0.40}
            for key in candidate
        },
        study.SP500_ID: {
            key: {**base, "annualized_return": 0.13}
            for key in candidate
        },
