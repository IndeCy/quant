...
from examples.intermediate_momentum_feasibility_study import (
    EXPERIMENT_ID as FEASIBILITY_ID,
    SOURCE_START,
    load_investable_source,
)
from examples.intermediate_momentum_report import render_report
...
STRATEGY_ID = "intermediate_momentum_12_1_v1"
REPORT_PATH = Path("docs/research/intermediate-momentum-12-1-v1.md")
...
RESEARCH_SPEC = ResearchSpec(
    ...
    hypothesis="跳过最近一个月的经典中期动量能否形成跨阶段可执行Alpha",
    definition={
        "factor": {
...
def _calculate(
    ...
    try:
        ...
        if not symbols:
            raise ValueError("12-1月动量没有产生历史持仓")
...
def build_diagnostics(holdings: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        "selected_momentum_median": float(
            holdings["intermediate_momentum"].median()
        ),
...
def _require_feasibility_passed(paths: RuntimePaths) -> None:
    ...
    if not latest or latest.get("status") != "SUCCESS":
        raise RuntimeError("12-1月动量数据可行性实验尚未成功完成")
    if latest.get("outcome") != "PASSED_FEASIBILITY":
        raise RuntimeError("12-1月动量数据可行性门禁未通过")
...
def _complete_attempt(
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "12-1月动量通过冻结门槛，仅允许进入独立前瞻确认"
            if passed
            else "12-1月动量未通过冻结门槛，终止且不注册生产"
        ),
        artifacts=[
