...
STRATEGY_ID = "insider_net_buying_v1"
REPORT_PATH = Path("docs/research/insider-net-buying-v1.md")
...
RESEARCH_SPEC = ResearchSpec(
    ...
    hypothesis="高管和个人重要股东的真实净增持是否形成稳定独立Alpha",
    definition={
        "factor": {
            "formula": "sum(IN change_ratio)-sum(DE change_ratio)",
            "lookback_calendar_days": LOOKBACK_DAYS,
            "eligible_holder_types": ["G", "P"],
            "excluded_holder_types": ["C"],
            "positive_net_buy_only": True,
            "direction": "higher_is_better",
            "transform": "winsorize_1_99_then_cross_sectional_percentile_rank",
        },
        "visibility": {
            "ann_date_lte_signal_date": True,
            "latest_visible_revision_per_event_plan": True,
            "event_identity": "symbol_holder_type_holder_direction_begin_date",
        },
        "source": {
            "provider": "tushare",
            "endpoint": "stk_holdertrade",
            "cache": "holder_trade_increment.duckdb",
        },
        "universe": {
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        materialize_holder_trade_asof(connection, lookback_days=LOOKBACK_DAYS)
        candidates = load_holder_trade_candidates(connection)
        targets, holdings, candidate_counts = build_holder_trade_targets(
            candidates,
            signal_dates,
        )
        ...
        if not symbols:
            raise ValueError("重要股东净增持因子没有产生历史持仓")
...
def load_holder_trade_candidates(connection: Any) -> pd.DataFrame:
    """连接标准股票池和重要股东增减持 as-of 截面。"""
    return connection.execute(
        ...
        name_asof AS (
            ...
            WHERE f.trade_date IN (SELECT signal_date FROM holder_trade_signal_dates)
        )
        SELECT
            f.trade_date AS signal_date,
            f.symbol,
            COALESCE(na.asof_name, sb.name) AS name,
            h.latest_event_ann_date,
            h.gross_buy_ratio,
            h.gross_sell_ratio,
            h.net_buy_ratio,
            h.buy_event_count,
            ...
            f.ret120
        FROM features f
        JOIN {HOLDER_TRADE_ASOF_TABLE} h
          ON f.trade_date = h.signal_date AND f.symbol = h.symbol
...
def build_holder_trade_targets(
    ...
    return targets, combined, {
        "min": float(count_series.min()) if not count_series.empty else 0.0,
        "median": float(count_series.median()) if not count_series.empty else 0.0,
        "latest": float(count_series.iloc[-1]) if not count_series.empty else 0.0,
        "months_below_top_n": float((count_series < TOP_N).sum()),
        "empty_months": float((count_series == 0).sum()),
    }
...
def build_momentum_diagnostics(
    ...
    """衡量净增持强度与120日动量的关系，仅用于归因。"""
    correlations: list[float] = []
    overlaps: list[float] = []
    for signal_date, group in candidates.groupby("signal_date", sort=True):
        valid = group.dropna(subset=["net_buy_ratio", "ret120"])
        positive = valid[valid["net_buy_ratio"].gt(0)]
        if len(positive) < 2:
            continue
        correlation = positive["net_buy_ratio"].rank().corr(positive["ret120"].rank())
        ...
        overlaps.append(len(selected & momentum) / denominator)
    return {
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "重要股东净增持通过固定样本外门槛，允许进入独立确认"
            if passed
            else "重要股东净增持未通过固定门槛，保留失败指纹且不注册生产策略"
        ),
        artifacts=[
...
- 数据截止：{latest_date}。
- 固定因子：过去{LOOKBACK_DAYS}天高管和个人股东增持比例减去减持比例。
...
- 与动量持仓重叠中位数：
  {diagnostics['median_holding_overlap_with_momentum']:.1%}，最新
  {diagnostics['latest_holding_overlap_with_momentum']:.1%}。
- 正净增持候选数：最少 {candidate_counts['min']:.0f}，中位数
  {candidate_counts['median']:.0f}，最新 {candidate_counts['latest']:.0f}；
  少于Top40共 {candidate_counts['months_below_top_n']:.0f} 个月，空仓
  {candidate_counts['empty_months']:.0f} 个月。
