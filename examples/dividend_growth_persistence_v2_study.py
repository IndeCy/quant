...
STRATEGY_ID = "dividend_growth_persistence_v2"
REPORT_PATH = Path("docs/research/dividend-growth-persistence-v2.md")
RESEARCH_SPEC = ResearchSpec(
    experiment_id=STRATEGY_ID,
    name="连续现金分红增长 V2",
    category="factor_strategy",
    hypothesis="剔除多年未再分红的陈旧历史后，连续现金分红增长是否仍具备稳定Alpha",
    definition={
        "base_definition": {
        ...
        "v2_data_quality_fix": {
            ...
            "reason": "annual_dividend_history_must_be_recent",
        },
        "visibility": {
            "dividend_process": "实施",
...
def apply_recent_dividend_history(frame: pd.DataFrame) -> pd.DataFrame:
    """只保留信号年前1至2个财年的最新已实施年报分红。"""
    required = ["signal_date", "latest_fiscal_year"]
    missing = [column for column in required if column not in frame.columns]
    if missing:
        raise ValueError(f"dividend recency frame missing columns: {missing}")
    data = frame.copy()
    signal_year = pd.to_numeric(
        data["signal_date"].astype(str).str[:4],
        errors="coerce",
    )
    fiscal_year = pd.to_numeric(data["latest_fiscal_year"], errors="coerce")
    data["latest_fiscal_year_age"] = signal_year - fiscal_year
    return data[
        data["latest_fiscal_year_age"].between(1, 2, inclusive="both")
    ].copy()
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        raw_candidates = load_dividend_growth_candidates(connection)
        candidates = apply_recent_dividend_history(raw_candidates)
        ...
        if not symbols:
            raise ValueError("连续现金分红增长 V2 没有产生任何历史持仓")
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "分红增长时效修复版通过固定样本外门槛，允许进入独立确认"
            if passed
            else "分红增长时效修复版未通过门槛，保留失败指纹且不注册生产策略"
        ),
        artifacts=[
