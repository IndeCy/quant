...
from examples.source_net_moneyflow_feasibility_report import render_report
from examples.source_net_moneyflow_feasibility_support import (
...
EXPERIMENT_ID = "source_net_moneyflow_data_feasibility_v1"
REPORT_PATH = Path("docs/research/source-net-moneyflow-data-feasibility-v1.md")
...
RESEARCH_SPEC = ResearchSpec(
    ...
    hypothesis="数据商净流入字段能否形成稳定、独立且可交易的月频候选池",
    definition={
        "factor": {
            "formula": (
                "2*sum(net_mf_amount,20d)/sum(all_classified_amount,20d)"
            ),
            "direction": "higher_is_better",
            "eligible": "positive_share_and_at_least_15_observations",
            "vendor_methodology": "opaque",
            "source_unit": "ten_thousand_cny",
        },
        "source_contract": {
            "all_bucket_buy_minus_sell": "approximately_zero",
            "classified_buy_plus_sell": "approximately_2x_market_turnover",
            "normalization_multiplier": 2.0,
        },
        "distinction": {
            "failed_large_bucket_v1": "uses_vendor_net_mf_not_bucket_difference",
            "signed_amount_pressure": "vendor_flow_not_price_direction_proxy",
        ...
        "frozen_gates": {
            ...
            "symmetric_bucket_share_min": 0.99,
            "classified_to_market_ratio": [1.80, [1.90, 2.10], 2.20],
            "source_share_range_violations": 0,
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    sync = update_order_flow_cache(
        TushareOrderFlowClient(token),
        cache_path,
        trade_dates,
    )
    if not sync.success:
        raise RuntimeError("订单流缓存存在未成功交易日")
    ...
    candidates = score_source_net_moneyflow_frame(source)
...
def build_monthly_coverage(
    ...
    for signal_date in expected_dates:
        ...
        factor = pd.to_numeric(
            group["source_net_turnover_share"],
            errors="coerce",
        )
...
def build_latest_distribution(candidates: pd.DataFrame) -> dict[str, float]:
    ...
    values = pd.to_numeric(
        latest["source_net_turnover_share"],
        errors="coerce",
    )
    return {
...
def evaluate_feasibility(
    ...
    """执行新指纹下冻结的数据契约、覆盖和独立性门槛。"""
    if monthly.empty:
        raise ValueError("官方源净流入覆盖审计没有预期月份")
    qualified = (
    ...
    checks = {
        ...
        "symmetric_bucket_contract": diagnostics["symmetric_bucket_share"] >= 0.99,
        "classified_amount_is_double_turnover": (
            diagnostics["turnover_ratio_p01"] >= 1.80
            and 1.90 <= diagnostics["turnover_ratio_median"] <= 2.10
            and diagnostics["turnover_ratio_p99"] <= 2.20
        ),
        ...
        "zero_source_share_range_violations": (
            diagnostics["source_share_range_violations"] == 0
        ),
    }
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "官方源净流入覆盖、契约和独立性通过，可进入固定回测"
            if result["passed"]
            else "官方源净流入覆盖、契约或独立性未通过"
        ),
        artifacts=[
            ExperimentArtifact("summary", summary_path, "源净流入可行性报告"),
...
def _data_version(paths: RuntimePaths, as_of_date: str) -> str:
    """绑定行情文件和已完整缓存的远端接口契约。"""
    parts = [f"tushare_moneyflow_daily:{FETCH_START}:{as_of_date}:vendor_net"]
    for label, path in [
