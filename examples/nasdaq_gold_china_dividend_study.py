...
from examples.nasdaq_gold_china_dividend_report import render_report
...
EXPERIMENT_ID = "nasdaq_gold_china_dividend_equal_v1"
REPORT_PATH = Path("docs/research/nasdaq-gold-china-dividend-equal-v1.md")
NASDAQ = "159941.SZ"
GOLD = "518880.SH"
DIVIDEND = "512890.SH"
BOND = "511010.SH"
SP500 = "513500.SH"
BENCHMARK = "510300.SH"
WEIGHTS = {NASDAQ: 1 / 3, GOLD: 1 / 3, DIVIDEND: 1 / 3}
...
NASDAQ_GOLD_ID = "nasdaq_gold_60_40_dividend_control"
GLOBAL_ID = "global_defensive_dividend_control"
SP500_ID = "sp500_direct_dividend_control"
DIVIDEND_ID = "dividend_lowvol_direct_control"
STRESS_ID = "nasdaq_gold_china_dividend_cost_20bps"
...
RESEARCH_SPEC = ResearchSpec(
    ...
    name="纳指×黄金×A股红利低波固定等权 V1",
    category="allocation_strategy",
    hypothesis=(
        "用A股红利低波ETF替代个股低频选股，和纳指增长、人民币黄金固定等权，"
        "能否在不牺牲场内标普机会成本的前提下获得跨风格分散"
    ),
    definition={
        "assets": {
            "growth": NASDAQ,
        ...
        "evaluation": {
            ...
            "return_shortfall_vs_nasdaq_gold_max": 0.04,
            "drawdown_worse_vs_nasdaq_gold_max": 0.02,
            "all_folds_positive": True,
            "worst_fold_drawdown_floor": -0.24,
        ...
        "methodology_version": "three_style_equal_multifold_v1",
    },
)
...
def _calculate(
    ...
    symbols = [NASDAQ, GOLD, DIVIDEND, BOND, SP500, BENCHMARK]
    ...
    if not audit["passed"]:
        raise ValueError(f"three-style panel audit failed: {audit}")
    signals = [
        date
    ...
    target_sets = {
        ...
        DIVIDEND_ID: _fixed_targets(signals, {DIVIDEND: 1.0}),
    }
...
def evaluate_gate(
    ...
    checks = {
        ...
        "return_shortfall_vs_nasdaq_gold_within_4pct": (
            nasdaq_gold["annualized_return"] - full["annualized_return"] <= 0.04
        ),
        "drawdown_worse_vs_nasdaq_gold_within_2pct": (
            abs(full["max_drawdown"]) - abs(nasdaq_gold["max_drawdown"]) <= 0.02
        ),
        "all_three_folds_positive": all(
            item["annualized_return"] > 0 for item in folds
        ),
        "worst_fold_drawdown_within_24pct": min(
...
def audit_panel(
    ...
    checks = {
        ...
        "at_least_1800_common_days": len(panel.calendar) >= 1800,
...
def _complete_attempt(
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "纳指黄金红利低波等权通过冻结门槛，仅允许前向研究观察"
            if passed
            else f"纳指黄金红利低波等权未通过门槛（{', '.join(failed)}），归档且不注册"
        ),
        artifacts=[
            ExperimentArtifact("summary", summary, "三风格等权研究报告"),
