...
from examples.long_term_reversal_feasibility_study import (
    EXPERIMENT_ID as FEASIBILITY_ID,
    SOURCE_START,
    load_investable_source,
)
from examples.long_term_reversal_report import render_report
...
STRATEGY_ID = "long_term_reversal_36_13_v1"
REPORT_PATH = Path("docs/research/long-term-reversal-36-13-v1.md")
STUDY_START = "20150101"
TOP_N = 40
RESEARCH_SPEC = ResearchSpec(
    experiment_id=STRATEGY_ID,
    name="长期反转 36 至 13 个月 V1",
    category="factor_strategy",
    hypothesis="长期过度反应后的价格修复能否形成跨阶段、低相关的A股Alpha",
    definition={
        "factor": {
        ...
        "prior_research_distinction": {
            "short_term_reversal": "excludes_most_recent_12_months",
            "momentum": "long_horizon_loser_rank_not_recent_winner_rank",
            "low_volatility": "return_level_not_return_dispersion",
        },
...
def build_diagnostics(holdings: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        "selected_long_return_median": float(
            holdings["long_term_return"].median()
        ),
...
def _require_feasibility_passed(paths: RuntimePaths) -> None:
    ...
    if not latest or latest.get("status") != "SUCCESS":
        raise RuntimeError("长期反转数据可行性实验尚未成功完成")
    if latest.get("outcome") != "PASSED_FEASIBILITY":
        raise RuntimeError("长期反转数据可行性门禁未通过")
...
def _complete_attempt(
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "长期反转通过固定四折门槛，仅允许进入独立前瞻确认"
            if passed
            else "长期反转未通过固定四折门槛，终止且不注册生产策略"
        ),
        artifacts=[
            ExperimentArtifact("summary", summary_path, "长期反转研究报告"),
