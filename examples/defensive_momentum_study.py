...
STRATEGY_ID = "defensive_momentum_v1"
REPORT_PATH = Path("docs/research/defensive-momentum-study.md")
TOP_N = 40
...
RESEARCH_SPEC = ResearchSpec(
    ...
    name="Defensive Momentum V1",
    category="factor_strategy",
    hypothesis="跳过近月的中期动量与低波动是否能形成独立且可执行的全A低频Alpha",
    definition={
        "factors": {
            "momentum_120_to_20d": {
                "weight": 0.50,
                "formula": "(1+ret120)/(1+ret20)-1",
                "positive_gate": True,
            },
            "low_volatility_60d": {"weight": 0.50, "formula": "-std(daily_return,60)"},
        },
        "diagnostics": {
            "momentum_only": {"momentum_weight": 1.0},
            "positive_momentum_low_volatility_only": {"momentum_weight": 0.0},
        },
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    low_vol_returns = runs["positive_momentum_low_volatility_only_v1"].result.daily_values.pct_change()
    momentum_returns = runs["skip_month_momentum_only_v1"].result.daily_values.pct_change()
    correlations = {
        "positive_momentum_low_volatility_only": float(main_returns.corr(low_vol_returns)),
        "momentum_only": float(main_returns.corr(momentum_returns)),
    }
...
def _build_selections(candidates: pd.DataFrame) -> tuple[dict[str, dict[str, list[str]]], pd.DataFrame]:
    mappings: dict[str, dict[str, list[str]]] = {}
    selected_frames: list[pd.DataFrame] = []
    variants = {
        "skip_month_momentum_only_v1": 1.0,
        "positive_momentum_low_volatility_only_v1": 0.0,
        STRATEGY_ID: 0.50,
    }
    for strategy_id, momentum_weight in variants.items():
        scored = _score_by_date(candidates, momentum_weight)
...
def _score_by_date(candidates: pd.DataFrame, momentum_weight: float) -> pd.DataFrame:
    frames: list[pd.DataFrame] = []
    for signal_date, group in candidates.groupby("signal_date", sort=True):
        scored = score_defensive_momentum_frame(group, momentum_weight=momentum_weight)
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "固定防御动量通过样本外门槛，允许进入独立确认"
            if passed
            else "固定防御动量未通过门槛，保留失败指纹且不注册生产策略"
        ),
        artifacts=[
...
def render_report(
    ...
    return f"""# Defensive Momentum V1 Study
...
- 与正中期动量池内低波单腿相关性：{correlations['positive_momentum_low_volatility_only']:.3f}。
- 与动量单腿相关性：{correlations['momentum_only']:.3f}。
