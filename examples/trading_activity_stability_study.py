...
from examples.trading_activity_stability_feasibility_study import (
    EXPERIMENT_ID as FEASIBILITY_ID,
    load_investable_source,
)
from examples.trading_activity_stability_report import render_report
...
STRATEGY_ID = "trading_activity_stability_v1"
REPORT_PATH = Path("docs/research/trading-activity-stability-v1.md")
...
RESEARCH_SPEC = ResearchSpec(
    ...
    hypothesis="较稳定的交易参与结构能否形成跨阶段、可执行的A股Alpha",
    definition={
        "factor": {
...
def _calculate(
    ...
    try:
        ...
        if not symbols:
            raise ValueError("交易活跃度稳定性没有产生历史持仓")
...
def build_diagnostics(holdings: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        "selected_activity_volatility_median": float(
            holdings["activity_volatility"].median()
        ),
...
def _require_feasibility_passed(paths: RuntimePaths) -> None:
    ...
    if not latest or latest.get("status") != "SUCCESS":
        raise RuntimeError("交易活跃度稳定性数据可行性实验尚未成功完成")
    if latest.get("outcome") != "PASSED_FEASIBILITY":
        raise RuntimeError("交易活跃度稳定性数据可行性门禁未通过")
...
def _complete_attempt(
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "交易活跃度稳定性通过冻结门槛，仅允许进入独立前瞻确认"
            if passed
            else "交易活跃度稳定性未通过冻结门槛，终止且不注册生产"
        ),
        artifacts=[
