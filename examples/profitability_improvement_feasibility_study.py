...
from data.profitability_improvement import (
    PROFITABILITY_IMPROVEMENT_TABLE,
...
from examples.profitability_improvement_feasibility_report import render_report
...
EXPERIMENT_ID = "profitability_improvement_data_feasibility_v1"
REPORT_PATH = Path(
    "docs/research/profitability-improvement-data-feasibility-v1.md"
)
...
RESEARCH_SPEC = ResearchSpec(
    ...
    name="年度 ROA 改善数据可行性 V1",
    category="data_feasibility",
    hypothesis="连续年报ROA改善能否形成广泛、独立且可交易的A股候选池",
    definition={
        "factor": {
            "formula": (
                "net_income_t/total_assets_t_minus_1-"
                "net_income_t_minus_1/total_assets_t_minus_2"
            ),
            "direction": "higher_is_better",
            "requires_three_consecutive_annual_balance_sheets": True,
        },
        "visibility": {
            "annual_1231_only": True,
            "income_and_balance_f_ann_date_lte_signal": True,
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        create_profitability_improvement_signal_dates(connection, signal_dates)
        attach_profitability_improvement_databases(
            connection,
            ProfitabilityImprovementPaths(
...
def load_investable_source(connection: Any) -> pd.DataFrame:
    ...
    return connection.execute(
        ...
        SELECT
            ...
            p.* EXCLUDE(signal_date, symbol),
            f.ret120,
            f.vol60,
            f.amount20 * 1000.0 AS adv_rmb
        FROM features f
        JOIN {PROFITABILITY_IMPROVEMENT_TABLE} p
...
def score_monthly_candidates(source: pd.DataFrame) -> pd.DataFrame:
    ...
    for signal_date, group in source.groupby("signal_date", sort=True):
        scored = score_profitability_improvement(group)
        ...
        rows.append(
            {
                ...
                "unique_factor_values": int(group["roa_change"].nunique()),
                ...
                "spearman_current_roa": _spearman(
                    group, "factor_score", "current_roa"
                ),
                "spearman_vol60": _spearman_low(
                    group, "factor_score", "vol60"
                ),
                "spearman_ret120": _spearman(
                    group, "factor_score", "ret120"
                ),
...
def evaluate_feasibility(
    ...
    """执行查看未来收益前冻结的数据、覆盖与排重门槛。"""
    if monthly.empty:
        raise ValueError("年度 ROA 改善没有月末截面")
    ...
    values = pd.to_numeric(candidates["roa_change"], errors="coerce")
    ...
    correlations = {
        "current_roa": float(monthly["spearman_current_roa"].median()),
    ...
    checks = {
        ...
        "distinct_from_current_roa": (
            abs(correlations["current_roa"]) <= MAX_STYLE_CORRELATION
        ),
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "年度ROA改善覆盖和独立性通过，可执行一次固定回测"
            if result["passed"]
            else "年度ROA改善数据、覆盖或排重门禁未通过"
        ),
        artifacts=[
