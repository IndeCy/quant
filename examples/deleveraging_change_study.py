...
from examples.deleveraging_change_feasibility_study import (
    EXPERIMENT_ID as FEASIBILITY_ID,
    STUDY_START,
    load_investable_source,
)
from examples.deleveraging_change_report import render_report
...
STRATEGY_ID = "deleveraging_change_v1"
REPORT_PATH = Path("docs/research/deleveraging-change-v1.md")
TOP_N = 40
RESEARCH_SPEC = ResearchSpec(
    experiment_id=STRATEGY_ID,
    name="年度去杠杆变化 V1",
    category="factor_strategy",
    hypothesis="资产负债率年度下降能否形成跨阶段可执行的独立Alpha",
    definition={
        "factor": {
            "formula": (
...
def _calculate(
    ...
    try:
        ...
        if not symbols:
            raise ValueError("年度去杠杆因子没有产生历史持仓")
...
def build_diagnostics(holdings: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        "selected_change_median": float(
            holdings["debt_to_assets_change"].median()
        ),
        "selected_leverage_median": float(
            holdings["debt_to_assets"].median()
        ),
...
def _require_feasibility_passed(paths: RuntimePaths) -> None:
    ...
    if not latest or latest.get("status") != "SUCCESS":
        raise RuntimeError("年度去杠杆数据可行性实验尚未成功完成")
    if latest.get("outcome") != "PASSED_FEASIBILITY":
        raise RuntimeError("年度去杠杆数据可行性门禁未通过")
...
def _complete_attempt(
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "年度去杠杆通过固定四折门槛，仅允许进入独立前瞻确认"
            if passed
            else "年度去杠杆未通过固定四折门槛，终止且不注册生产"
        ),
        artifacts=[
