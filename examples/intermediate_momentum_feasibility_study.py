...
from data.intermediate_momentum import (
    INTERMEDIATE_MOMENTUM_TABLE,
...
EXPERIMENT_ID = "intermediate_momentum_12_1_data_feasibility_v1"
REPORT_PATH = Path(
    "docs/research/intermediate-momentum-12-1-data-feasibility-v1.md"
)
SOURCE_START = "20131201"
TOP_N = 40
...
MAX_EXISTING_MOMENTUM_CORRELATION = 0.85
MAX_VOLATILITY_CORRELATION = 0.80
RESEARCH_SPEC = ResearchSpec(
    experiment_id=EXPERIMENT_ID,
    name="12-1月中期动量数据可行性 V1",
    category="data_feasibility",
    hypothesis="经典12-1月动量能否形成广泛且不等同于既有120日平滑动量的候选池",
    definition={
        "factor": {
            "formula": "qfq_close_lag20/qfq_close_lag252-1",
            "direction": "higher_is_better",
            "source": "daily_adj_cache",
        },
        "distinction_from_existing": {
            "existing": "120d_skip20_momentum_times_continuity_positive_gate",
            "candidate": "252d_skip20_raw_return_without_positive_gate",
        },
        ...
        "frozen_gates": {
            ...
            "maximum_ret120_rank_correlation": (
                MAX_EXISTING_MOMENTUM_CORRELATION
            ),
            "maximum_vol60_rank_correlation": MAX_VOLATILITY_CORRELATION,
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        materialize_intermediate_momentum(connection)
        latest_date = str(
        ...
        connection.close()
    candidates = score_intermediate_momentum(source)
...
def load_investable_source(
    ...
    """使用标准历史名称、上市年限和流动性规则构造截面。"""
    connection.execute(
        "CREATE OR REPLACE TEMP TABLE momentum_signal_dates(signal_date VARCHAR)"
    )
    connection.executemany(
        "INSERT INTO momentum_signal_dates VALUES (?)",
    ...
    return connection.execute(
        ...
        name_asof AS (
            ...
            WHERE f.trade_date IN (SELECT signal_date FROM momentum_signal_dates)
        )
        SELECT
            f.trade_date AS signal_date,
            f.symbol,
            ...
            m.close_skip_lag,
            m.close_formation_lag,
            m.intermediate_momentum,
            f.ret120,
            f.vol60,
            f.amount20 * 1000.0 AS adv_rmb
        FROM features f
        JOIN {INTERMEDIATE_MOMENTUM_TABLE} m
        ...
        rows.append(
            {
                ...
                "unique_factor_values": int(
                    group["intermediate_momentum"].nunique()
                ),
...
def evaluate_feasibility(
    ...
    """应用读取未来收益前冻结的门禁。"""
    if monthly.empty:
        raise ValueError("12-1月动量没有月末截面")
    valid = candidates.dropna(
        subset=["close_skip_lag", "close_formation_lag", "intermediate_momentum"]
    )
    expected = valid["close_skip_lag"] / valid["close_formation_lag"] - 1
    identity_error = (valid["intermediate_momentum"] - expected).abs()
    duplicate_rows = int(candidates.duplicated(["signal_date", "symbol"]).sum())
    invalid_rows = int(
        pd.to_numeric(candidates["intermediate_momentum"], errors="coerce")
        .isna()
        .sum()
    )
    qualified = (
    ...
    checks = {
        ...
        "distinct_from_existing_120d_momentum": (
            abs(ret_correlation) <= MAX_EXISTING_MOMENTUM_CORRELATION
        ),
        "distinct_from_low_volatility": (
            abs(vol_correlation) <= MAX_VOLATILITY_CORRELATION
        ),
...
- Top40 日均成交额中位数：{result['top40_adv_median_rmb']:,.0f} 元。
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "12-1月动量覆盖和差异性通过，可执行一次固定回测"
            if result["passed"]
            else "12-1月动量覆盖、差异性或可交易性未通过"
        ),
        artifacts=[
