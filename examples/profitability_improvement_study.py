...
from examples.operating_cashflow_stability_study import evaluate_gate
from examples.profitability_improvement_feasibility_study import (
    EXPERIMENT_ID as FEASIBILITY_ID,
    STUDY_START,
    load_investable_source,
)
from examples.profitability_improvement_report import render_report
...
STRATEGY_ID = "profitability_improvement_v1"
REPORT_PATH = Path("docs/research/profitability-improvement-v1.md")
TOP_N = 40
RESEARCH_SPEC = ResearchSpec(
    experiment_id=STRATEGY_ID,
    name="年度 ROA 改善 V1",
    category="factor_strategy",
    hypothesis="连续年报ROA改善能否形成跨阶段、可执行且独立的A股Alpha",
    definition={
        "factor": {
            "formula": (
...
def _calculate(
    ...
    try:
        ...
        if not symbols:
            raise ValueError("年度 ROA 改善因子没有产生历史持仓")
...
def build_diagnostics(holdings: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        "selected_change_median": float(holdings["roa_change"].median()),
        "selected_current_roa_median": float(
            holdings["current_roa"].median()
        ),
        "selected_prior_roa_median": float(holdings["prior_roa"].median()),
...
def _require_feasibility_passed(paths: RuntimePaths) -> None:
    ...
    if not latest or latest.get("status") != "SUCCESS":
        raise RuntimeError("年度 ROA 改善数据可行性实验尚未成功完成")
    if latest.get("outcome") != "PASSED_FEASIBILITY":
        raise RuntimeError("年度 ROA 改善数据可行性门禁未通过")
...
def _complete_attempt(
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "年度ROA改善通过固定四折门槛，仅允许进入独立前瞻确认"
            if passed
            else "年度ROA改善未通过固定四折门槛，终止且不注册生产"
        ),
        artifacts=[
