...
STRATEGY_ID = "low_asset_growth_v1"
REPORT_PATH = Path("docs/research/low-asset-growth-v1.md")
...
RESEARCH_SPEC = ResearchSpec(
    ...
    hypothesis="资本扩张更保守的公司是否形成稳定独立的投资因子Alpha",
    definition={
        "field_semantic_audit": {
            "vendor_field": "fina_indicator.assets_yoy",
            "raw_formula": "(annual_total_assets/prior_annual_total_assets-1)*100",
            "matched_rows": 80731,
            "correlation": 0.997776,
            "median_absolute_error_pct_point": 0.000029,
            "within_0_1_pct_point_ratio": 0.924354,
        },
        "factor": {
            "formula": "annual_total_assets_yoy_pct",
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        candidates = apply_asset_growth_universe(raw)
        targets, holdings, candidate_counts = build_asset_growth_targets(
            candidates,
            signal_dates,
        )
        ...
        if not symbols:
            raise ValueError("低资产增长因子没有产生历史持仓")
...
def apply_asset_growth_universe(frame: pd.DataFrame) -> pd.DataFrame:
    """应用可交易过滤，但不要求任何 Quality 因子或分位数。"""
    required = [
        "signal_date",
        "symbol",
        "name",
    ...
    if missing:
        raise ValueError(f"asset growth candidates missing columns: {missing}")
    data = frame.copy()
...
def build_diagnostics(
    ...
    """衡量投资因子与动量的关系及选股资产增长暴露。"""
    correlations: list[float] = []
    overlaps: list[float] = []
    for signal_date, group in candidates.groupby("signal_date", sort=True):
        valid = group.dropna(subset=["assets_yoy", "ret120"])
        if len(valid) < TOP_N:
            continue
        low_growth = -valid["assets_yoy"].rank()
        correlation = low_growth.corr(valid["ret120"].rank())
    ...
    return {
        ...
        "selected_assets_yoy_median": float(
            pd.to_numeric(holdings["assets_yoy"], errors="coerce").median()
        ),
        "universe_assets_yoy_median": float(
            pd.to_numeric(candidates["assets_yoy"], errors="coerce").median()
        ),
    }
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "低资产增长通过固定样本外门槛，允许进入独立确认"
            if passed
            else "低资产增长未通过固定门槛，保留失败指纹且不注册生产策略"
        ),
        artifacts=[
...
- 与120日动量Spearman中位数：
  {diagnostics['median_spearman_with_ret120']:.3f}；Top40重叠
  {diagnostics['median_top40_overlap_with_momentum']:.1%}。
- 持仓资产增长中位数：{diagnostics['selected_assets_yoy_median']:.2f}%，
  全候选中位数：{diagnostics['universe_assets_yoy_median']:.2f}%。
