...
from examples.deleveraging_change_feasibility_report import render_report
...
EXPERIMENT_ID = "deleveraging_change_data_feasibility_v1"
REPORT_PATH = Path("docs/research/deleveraging-change-data-feasibility-v1.md")
...
RESEARCH_SPEC = ResearchSpec(
    ...
    name="年度去杠杆变化数据可行性 V1",
    category="data_feasibility",
    hypothesis="资产负债率年度下降能否形成广泛、独立且可交易的候选池",
    definition={
        "factor": {
            "formula": (
                "total_liabilities_t/total_assets_t-"
                "total_liabilities_t_minus_1/total_assets_t_minus_1"
            ),
            "direction": "lower_is_better",
            "requires_two_consecutive_annual_reports": True,
        },
        "visibility": {
            "annual_1231_only": True,
            "latest_revision_f_ann_date_lte_signal": True,
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        create_deleveraging_signal_dates(connection, signal_dates)
        attach_deleveraging_database(
            connection,
            DeleveragingPaths(paths.balance_sheet_path),
        )
    ...
    return connection.execute(
        ...
        name_asof AS (
            ...
            WHERE f.trade_date IN (
                SELECT signal_date FROM deleveraging_signal_dates
            )
        )
        SELECT
            f.trade_date AS signal_date,
        ...
        FROM features f
        JOIN {DELEVERAGING_CHANGE_TABLE} d
...
def score_monthly_candidates(source: pd.DataFrame) -> pd.DataFrame:
    ...
    for signal_date, group in source.groupby("signal_date", sort=True):
        scored = score_deleveraging_change(group)
        ...
        rows.append(
            {
                ...
                "unique_factor_values": int(
                    group["debt_to_assets_change"].nunique()
                ),
                ...
                "spearman_low_leverage": _spearman_low(
                    group,
                    "factor_score",
                    "debt_to_assets",
                ),
...
def evaluate_feasibility(
    ...
    """执行查看未来收益前冻结的数据与排重门槛。"""
    if monthly.empty:
        raise ValueError("年度去杠杆变化没有月末截面")
    duplicate_rows = int(
    ...
    values = pd.to_numeric(
        candidates["debt_to_assets_change"],
        errors="coerce",
    )
    ...
    correlations = {
        "low_leverage": float(
            monthly["spearman_low_leverage"].median()
        ),
    ...
    checks = {
        ...
        "distinct_from_low_leverage": (
            abs(correlations["low_leverage"]) <= MAX_STYLE_CORRELATION
        ),
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "年度去杠杆覆盖和独立性通过，可执行一次固定回测"
            if result["passed"]
            else "年度去杠杆数据、排重或可交易性门禁未通过"
        ),
        artifacts=[
