...
from examples.net_debt_financing_feasibility_study import (
    STUDY_START,
    load_candidates,
)
...
STRATEGY_ID = "net_debt_financing_strict_st_v2"
REPORT_PATH = Path("docs/research/net-debt-financing-strict-st-v2.md")
TOP_N = 40
RESEARCH_SPEC = ResearchSpec(
    experiment_id=STRATEGY_ID,
    name="净债务融资严格ST过滤 V2",
    category="factor_strategy",
    hypothesis="一般工商业年度净偿债现金流是否形成稳定独立Alpha",
    definition={
        "correction_from": "net_debt_financing_v1",
        "correction": {
            "name_filter": "duckdb_regexp_matches_ST_or_delisted",
            "reason": "V1_SIMILAR_TO_did_not_match_substrings",
            "all_factor_portfolio_risk_execution_rules_unchanged": True,
        },
        "source": {
            "reference": "tushare_cashflow_doc_44",
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not symbols:
            raise ValueError("净债务融资因子没有产生历史持仓")
...
def build_targets(
    ...
    for signal_date, group in candidates.groupby("signal_date", sort=True):
        scored = score_net_debt_financing_frame(group)
...
def build_diagnostics(
    ...
    """衡量债务活动、动量重叠和截止位并列。"""
    activity_correlations: list[float] = []
    momentum_overlaps: list[float] = []
    cutoff_ties: list[int] = []
    for signal_date, group in candidates.groupby("signal_date", sort=True):
        scored = score_net_debt_financing_frame(group)
        valid = scored.dropna(subset=["gross_debt_activity", "ret120"])
        if len(valid) < TOP_N:
            continue
        correlation = valid["net_debt_financing"].rank().corr(
            valid["gross_debt_activity"].rank()
        )
        if pd.notna(correlation):
            activity_correlations.append(float(correlation))
        selected = set(
            holdings.loc[
        ...
        cutoff = valid["net_debt_financing"].nsmallest(TOP_N).iloc[-1]
        cutoff_ties.append(int(valid["net_debt_financing"].eq(cutoff).sum()))
    return {
        "median_spearman_with_gross_debt_activity": float(
            pd.Series(activity_correlations).median()
        ),
        "median_top40_overlap_with_momentum": float(
            pd.Series(momentum_overlaps).median()
        ),
        "maximum_cutoff_tie_count": float(max(cutoff_ties)),
        "selected_net_debt_financing_median": float(
            holdings["net_debt_financing"].median()
        ),
        "selected_all_components_missing_share": float(
            holdings["all_debt_cashflow_missing"].mean()
        ),
    }
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "净债务融资通过固定门槛，允许进入独立确认"
            if passed
            else "净债务融资未通过固定门槛，终止且不注册生产策略"
        ),
        artifacts=[
            ExperimentArtifact("summary", summary_path, "净债务融资研究报告"),
