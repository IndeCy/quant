...
REPORT_PATH = Path("docs/research/quality-tradability-extension-study.md")
TOP_N = 20
...
CANDIDATES = (
    MarketCandidate(
        ...
        "Quality Liquidity Extension V1",
        {"log_amount_20d": 0.15},
    ),
    MarketCandidate(
        "quality_downside_risk_extension_v1",
        "Quality Downside Risk Extension V1",
        {"low_downside_volatility_60d": 0.15},
    ),
    MarketCandidate(
        "quality_liquidity_downside_v1",
        "Quality Liquidity Downside V1",
        {"log_amount_20d": 0.10, "low_downside_volatility_60d": 0.10},
    ),
    MarketCandidate(
        "quality_liquidity_tail_v1",
        "Quality Liquidity Tail V1",
        {"log_amount_20d": 0.10, "worst_day_stability_60d": 0.10},
    ),
    MarketCandidate(
        "quality_downside_tail_v1",
        "Quality Downside Tail V1",
        {"low_downside_volatility_60d": 0.10, "worst_day_stability_60d": 0.10},
    ),
)
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        source = _add_tradability_factors(source)
...
def _add_tradability_factors(source: pd.DataFrame) -> pd.DataFrame:
    result = source.copy()
    amount = pd.to_numeric(result["amount20"], errors="coerce").clip(lower=0)
    result["log_amount_20d"] = amount.map(
        lambda value: math.log1p(value) if pd.notna(value) else float("nan")
    )
    result["low_downside_volatility_60d"] = -pd.to_numeric(
        result["downside_vol60"],
        errors="coerce",
    )
    result["worst_day_stability_60d"] = pd.to_numeric(
        result["worst_ret60"],
        errors="coerce",
    )
    return result
...
def render_report(
    ...
    return f"""# Quality Tradability Extension Study
