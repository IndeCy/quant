...
STRATEGY_ID = "shareholder_concentration_v1"
REPORT_PATH = Path("docs/research/shareholder-concentration-v1.md")
TOP_N = 40
MIN_GAP_DAYS = 20
MAX_GAP_DAYS = 180
...
RESEARCH_SPEC = ResearchSpec(
    ...
    hypothesis="股东户数持续下降所代表的筹码集中，能否形成稳定独立Alpha",
    definition={
        "factor": {
            "formula": "-log(latest_holder_num/prior_holder_num)*90/gap_days",
            "direction": "higher_is_better",
            "period_gap_days": [MIN_GAP_DAYS, MAX_GAP_DAYS],
            "max_staleness_days": MAX_STALENESS_DAYS,
            "transform": "winsorize_1_99_then_cross_sectional_percentile_rank",
        },
        "visibility": {
            "ann_date_lte_signal_date": True,
            "end_date_lte_signal_date": True,
            "latest_visible_revision_per_end_date": True,
        },
        "source": {
            "provider": "tushare",
            "endpoint": "stk_holdernumber",
            "cache": "shareholder_count_increment.duckdb",
        },
        "universe": {
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        create_shareholder_signal_date_table(connection, signal_dates)
        attach_shareholder_count_database(connection, paths.shareholder_count_path)
        materialize_shareholder_concentration_asof(
            connection,
        ...
        candidates = load_shareholder_candidates(connection)
        targets, holdings, candidate_counts = build_shareholder_targets(candidates)
        diagnostics = build_momentum_diagnostics(candidates, holdings)
        symbols = sorted(holdings["symbol"].astype(str).unique().tolist())
        if not symbols:
            raise ValueError("股东户数筹码集中因子没有产生历史持仓")
...
def load_shareholder_candidates(connection: Any) -> pd.DataFrame:
    """连接标准股票池和股东户数 as-of 截面。"""
    return connection.execute(
        ...
        name_asof AS (
            ...
            WHERE f.trade_date IN (SELECT signal_date FROM shareholder_signal_dates)
        )
        SELECT
            f.trade_date AS signal_date,
            f.symbol,
            ...
            s.period_gap_days,
            s.staleness_days,
            s.concentration_rate_90d,
            f.vol60,
            f.ret20,
            f.ret120
        FROM features f
        JOIN {SHAREHOLDER_ASOF_TABLE} s
...
def build_momentum_diagnostics(
    ...
    overlaps: list[float] = []
    for signal_date, group in candidates.groupby("signal_date", sort=True):
        valid = group.dropna(subset=["concentration_rate_90d", "ret120"])
        if len(valid) < TOP_N:
            continue
        holder_rank = valid["concentration_rate_90d"].rank(method="average")
        momentum_rank = valid["ret120"].rank(method="average")
        correlation = holder_rank.corr(momentum_rank)
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "股东户数筹码集中通过固定样本外门槛，允许进入独立确认"
            if passed
            else "股东户数筹码集中未通过固定门槛，保留失败指纹且不注册生产策略"
        ),
        artifacts=[
...
def render_report(
    ...
    return f"""# 股东户数筹码集中 V1

- 数据截止：{latest_date}。
- 固定因子：最近两次披露户数的90日标准化下降速度，越高越好。
- 披露期相隔{MIN_GAP_DAYS}至{MAX_GAP_DAYS}天，最新期不超过
  {MAX_STALENESS_DAYS}天，公告日和报告期均不得晚于信号日。
...
def _data_version(paths: RuntimePaths) -> str:
    ...
    parts = []
    for label, path in [
        ("shareholder", paths.shareholder_count_path),
