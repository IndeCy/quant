...
STRATEGY_ID = "intraday_strength_vs_overnight_20d_v1"
REPORT_PATH = Path("docs/research/intraday-strength-vs-overnight-20d-v1.md")
WINDOW = 20
TOP_N = 40
RESEARCH_SPEC = ResearchSpec(
    ...
    hypothesis="剔除隔夜情绪溢价并保留盘中持续买盘，能否形成独立且可执行的A股Alpha",
    definition={
        "factor": {
            "formula": "sum_20d(log(qfq_close/qfq_open))-sum_20d(log(qfq_open/prev_qfq_close))",
        ...
        "source": {
            ...
            "fields": ["open_qfq", "close_qfq"],
            "decomposition_identity_required": True,
            "future_data": False,
        },
        "universe": {
...
def _calculate(paths: RuntimePaths, as_of_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        materialize_intraday_strength(connection, window=WINDOW)
        latest_date = str(
        ...
        if not data_quality["passed"]:
            raise ValueError(f"收益分解数据质量未通过: {data_quality}")
        ...
        if not symbols:
            raise ValueError("日内强度因子没有产生历史持仓")
...
def load_candidates(
    ...
    """连接标准可交易股票池与同日收益时段分解特征。"""
    connection.execute(
        "CREATE OR REPLACE TEMP TABLE intraday_signal_dates(signal_date VARCHAR)"
    )
    connection.executemany(
        "INSERT INTO intraday_signal_dates VALUES (?)",
    ...
    return connection.execute(
        ...
        name_asof AS (
            ...
            WHERE f.trade_date IN (SELECT signal_date FROM intraday_signal_dates)
        )
        SELECT
            f.trade_date AS signal_date,
            f.symbol,
            COALESCE(na.asof_name, sb.name) AS name,
            i.intraday_log_return_20,
            i.overnight_log_return_20,
            i.intraday_strength,
            i.decomposed_total_return,
            f.ret20,
            f.ret120,
            f.vol60
        FROM features f
        JOIN {INTRADAY_STRENGTH_TABLE} i
          ON f.trade_date = i.trade_date AND f.symbol = i.symbol
...
def validate_data_quality(candidates: pd.DataFrame) -> dict[str, Any]:
    """验证覆盖和收益分解恒等式，避免窗口错位产生虚假信号。"""
    if candidates.empty:
        return {"passed": False, "max_identity_error": None, "months": 0}
    identity_error = (
        candidates["decomposed_total_return"] - candidates["ret20"]
    ).abs()
    counts = candidates.groupby("signal_date")["symbol"].nunique()
    maximum_error = float(identity_error.max())
    return {
        "passed": bool(maximum_error <= 1e-8 and counts.min() >= TOP_N),
...
def build_diagnostics(
    ...
    """衡量新因子与总收益动量、波动率的重合程度。"""
    momentum_correlations: list[float] = []
    volatility_correlations: list[float] = []
    overlaps: list[float] = []
    for signal_date, group in candidates.groupby("signal_date", sort=True):
        valid = group.dropna(subset=["intraday_strength", "ret20", "vol60"])
        if len(valid) < TOP_N:
            continue
        momentum = valid["intraday_strength"].rank().corr(valid["ret20"].rank())
        volatility = valid["intraday_strength"].rank().corr(valid["vol60"].rank())
        if pd.notna(momentum):
            momentum_correlations.append(float(momentum))
        if pd.notna(volatility):
            volatility_correlations.append(float(volatility))
        selected = set(
            holdings.loc[
        ...
        momentum_top = set(valid.nlargest(TOP_N, "ret20")["symbol"].astype(str))
        overlaps.append(len(selected & momentum_top) / TOP_N)
    return {
        "median_spearman_with_ret20": _median(momentum_correlations),
        "median_spearman_with_vol60": _median(volatility_correlations),
        "median_top40_overlap_with_ret20": _median(overlaps),
        "latest_top40_overlap_with_ret20": overlaps[-1] if overlaps else float("nan"),
    }
...
def _complete_attempt(attempt: ResearchAttempt, result: dict[str, Any]) -> None:
    ...
    complete_research_attempt(
        ...
        decision_reason=(
            "日内强度减隔夜情绪通过固定样本外门槛，允许进入独立确认"
            if passed
            else "日内强度减隔夜情绪未通过固定门槛，归档且不注册生产策略"
        ),
        artifacts=[
...
- 与20日总收益Spearman中位数：
  {diagnostics['median_spearman_with_ret20']:.3f}；Top40重叠中位数：
  {diagnostics['median_top40_overlap_with_ret20']:.1%}。
- 与60日波动率Spearman中位数：
