...
REPORT_PATH = Path("docs/research/quality-extension-factor-study.md")
TOP_N = 20
...
class ExtensionCandidate:
    ...
    def research_spec(self) -> ResearchSpec:
        ...
        return ResearchSpec(
            ...
            hypothesis="单一财务维度能否稳定增强Quality V1而不放大执行风险",
            definition={
                "factors": {
                    factor: {
                        "direction": 1,
...
CANDIDATES = (
    ExtensionCandidate(
        "quality_roic_extension_v1",
        "Quality ROIC Extension V1",
        (*BASE_FACTORS, "roic"),
    ),
    ExtensionCandidate(
        "quality_margin_extension_v1",
        "Quality Margin Extension V1",
        (*BASE_FACTORS, "netprofit_margin", "grossprofit_margin"),
    ),
    ExtensionCandidate(
        "quality_cash_verification_v1",
        "Quality Cash Verification V1",
        (*BASE_FACTORS, "salescash_to_or", "dtprofit_to_profit"),
    ),
    ExtensionCandidate(
        "quality_profit_growth_v1",
        "Quality Profit Growth V1",
        (*BASE_FACTORS, "netprofit_yoy"),
    ),
    ExtensionCandidate(
        "quality_revenue_growth_v1",
        "Quality Revenue Growth V1",
        (*BASE_FACTORS, "tr_yoy"),
    ),
)
...
def evaluate_gate(
    ...
    checks = {
        ...
        "locked_test_drawdown_within_25pct": test["max_drawdown"] >= -0.25,
        "locked_test_positive_excess": test["excess_return"] > 0,
        "locked_test_sharpe_near_quality_v1": (
            test["sharpe"] >= baseline["locked_test"]["sharpe"] - 0.05
        ),
...
def _build_selections(
    ...
    for candidate in CANDIDATES:
        scored = _score(
            source,
            {factor: 1 for factor in candidate.factors},
        )
...
def _score(
    ...
    for signal_date, group in source.groupby("signal_date", sort=True):
        if factors is None:
            ...
            scored = score_quality_fundamental_frame(group, factors)
...
def render_report(
    ...
    return f"""# Quality Extension Factor Study
